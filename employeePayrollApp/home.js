...
const createInnerHtml = () => { 
    ...
    for (const empPayrollData of empPayrollList) { 
        innerHtml = `${innerHtml} 
        <tr>
            <td><img class="profile" src="${empPayrollData._profilePic}" width="30px" height="30px" alt=""></td>
            <td>${empPayrollData._name}</td> 
            <td>${empPayrollData._gender}</td> 
            <td>${getDeptHtml(empPayrollData._department)}</td> 
            <td>${empPayrollData._salary}</td> 
            <td>${convertToString(empPayrollData._startDate)}</td> 
        <td>
...
function convertToString(date){
    ...
    const newDate = !date ? "undefined" : new Date(Date.parse(date)).toLocaleDateString('en-GB',opt);
    return newDate
}
const remove = (node) => {
